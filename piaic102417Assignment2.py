...
def function3():
    ...
    x = a[np.where((np.mod(a,5)==0)&(np.mod(a,7)==0))] #wrtie your code here
    return x
    """
    Expected Output:
...
def function11():
    ...
    ans = np.where(np.mod(arr,2)>0,-1,arr) #write your code here 
...
def function15():
    ...
    ans = mat[np.sort(mat[:,1]).argsort()] #write your code here 
...
def function16():
    ...
    x = np.array([[1], [2], [3]])
    y = np.array([[2], [3], [4]])
    ans = np.array(np.hsplit(np.hstack ((x,y)),1)) #write your code here 
